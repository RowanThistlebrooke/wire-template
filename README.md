...
The main student flow has been exercised, including a real Claude Desktop tool conversation. [Update 004](COURSE-UPDATES.md#update-004--safe-display-and-comparison-states--completed-projects) includes display escaping and comparison-state fixes, checked with existing real data, browser rendering, and a read-only MCP session. The checked anonymous requests were denied, and checked signed-in reads returned that user's rows; an independent second-account isolation test remains outstanding. Final checks also remain for wider data/error cases, CSV dates and formats, large histories, a successful scheduled run, and scoring consistency. See the updates page for the current state.
